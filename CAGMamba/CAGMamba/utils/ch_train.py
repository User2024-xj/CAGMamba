...
def ChRun(config):
    ...
    # 定义固定的模型保存路径（覆盖模式）
    best_loss_model_path = config.model_save_path + f'CH_best_loss_{config.dataset_name}_{config.seed}.pth'
    best_acc_model_path = config.model_save_path + f'CH_best_acc_{config.dataset_name}_{config.seed}.pth'

    print(f"最佳损失模型保存路径: {best_loss_model_path}")
    print(f"最佳准确率模型保存路径: {best_acc_model_path}")
