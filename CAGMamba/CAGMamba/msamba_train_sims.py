# ====================================================================================
# 文件: msamba_train_sims.py - SIMS数据集训练逻辑（完整Mamba实现版本）
# ====================================================================================

"""
MSAmba-MMML训练逻辑 - SIMS数据集版本（完整Mamba实现）
适配中文数据集的训练流程，保持完整Mamba架构
"""
import torch
...
try:
    ...
    from msamba_mmml_model_sims import MSAmba_MMML_SIMS, MSAmbaSimsConfig  # SIMS版本模型
    print("✅ 成功导入完整Mamba MSAmba-SIMS训练组件")
    ...
    try:
        ...
        from msamba_mmml_model_sims import MSAmba_MMML_SIMS, MSAmbaSimsConfig
        print("✅ 从当前目录导入完整Mamba MSAmba-SIMS训练组件")
    except ImportError:
...
def save_test_results_sims(test_results, config, model_identifier):
    """保存SIMS测试结果到JSON文件（添加Mamba参数信息）"""
    ...
    save_data = {
        ...
        'model_type': 'MSAmba_SIMS_Full_Mamba_Implementation',
        'language': 'Chinese',
        'architecture_info': {
    ...
    try:
        ...
        print(f"✅ SIMS测试结果已保存到: {results_path}")
        ...
        print(f"✅ SIMS简化测试指标已保存到: {simple_results_path}")
        
    except Exception as e:
        print(f"❌ 保存SIMS测试结果失败: {e}")
        
    return results_path


class MSAmbaSimsConfig_Training(object):
    """MSAmba-MMML完整Mamba配置类 - SIMS版本，添加Mamba核心参数"""
    def __init__(self,
                ...
                loss_weights = {
                 ...
                 dropout=0.3,
                 model='msamba',
                 batch_size = 16,  # SIMS通常用较大的batch size
                 multi_task = True,
                 tasks = 'MTA',   # 'M' or 'MTA'
...
class MSAmbaSimsTrainer():
    ...
    def do_train(self, model, data_loader):    
        ...
        # 中文Hubert参数
        hubert_params = []
        for name, param in model.hubert_model.named_parameters():
            if param.requires_grad:
        ...
        for name, param in model.named_parameters():
            if param.requires_grad and not any([
                ...
                'hubert_model' in name,
                'chm_layers' in name
            ]):
                other_params.append(param)
        
        # 打印参数统计
        print(f"中文RoBERTa decay params: {len(bert_params_decay)}")
        print(f"中文RoBERTa no-decay params: {len(bert_params_no_decay)}")
        print(f"中文Hubert params: {len(hubert_params)}")
        print(f"=== Mamba参数统计（SIMS版本）===")
        ...
        if hubert_params:
            param_groups.append({'params': hubert_params, 'weight_decay': 0.01, 'lr': 5e-6})
        ...
        for batch in tqdm(data_loader, desc="Training SIMS"):                    
            ...
            targets = batch["targets"]  # SIMS的targets是字典格式
        ...
        # 计算平均损失
        for m in self.tasks:
            train_loss[m] = round(train_loss[m] / input_size, 4)
    ...
    def do_test(self, model, data_loader, mode):
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader, desc=f"SIMS {mode} Evaluation"):
                text_inputs = batch["text_tokens"].to(device)
                audio_inputs = batch["audio_inputs"].to(device)
                text_mask = batch["text_masks"].to(device)
                audio_mask = batch["audio_masks"].to(device)
        ...
        eval_results = eval_results[self.tasks[0]]  # 使用第一个任务的结果作为主要指标
        eval_results['Loss'] = total_loss
        
        return eval_results


def MSAmbaSimsRun(config):
    """MSAmba-MMML完整Mamba训练主函数 - SIMS版本"""
    ...
    # === 修改：创建完整Mamba配置（SIMS版本）===
    msamba_config = MSAmbaSimsConfig(
        dropout=config.dropout,
        chm_depth=config.chm_depth,
        use_checkpoint=config.use_checkpoint,
        text_model_path=config.text_model_path,
    ...
    # 创建使用完整Mamba的SIMS模型
    model = MSAmba_MMML_SIMS(msamba_config).to(device)
    
    # 冻结Hubert特征提取器
    for param in model.hubert_model.feature_extractor.parameters():
        param.requires_grad = False

    trainer = MSAmbaSimsTrainer(config)
    
    # === 修改：生成包含Mamba参数的模型文件名 ===
    # 构建详细的模型标识符（包含Mamba参数）
    model_identifier = (f'MSAmba_SIMS_FullMamba_'
    ...
    print(f"SIMS模型标识: {model_identifier}")
    ...
    #         best_epoch = epoch
    #         print(f"✅ 保存SIMS最佳损失模型: {lowest_eval_loss:.4f}")
        
    #     # 保存最佳准确率模型（SIMS使用Mult_acc_2指标）
